# BMSTU Operating Systems course

## Лабораторные работы

### 1st semester

1. [int8h, приоритеты процессов](semester1/lab1)
2. [Перевод компьютера в защищенный режим](semester1/lab2/p.asm)  
3. [Командная строка Linux](semester1/lab3/main.c)
4. [Системные вызовы fork, wait, exec, pipe, signal](semester1/lab4)
5. [Задачи "производство-потребление" и "читатели-писатели" на семафорах](semester1/lab5)
6. [Задача "читатели-писатели" для Windows](semester1/lab6/main.c)  

### 2nd semester

1. [Демон](semester2/lab1/main.c)
2. [Дерево файлов](semester2/lab2)
3. [Загружаемые модули ядра](semester2/lab3_kernel_modules)
4. [Виртуальная файловая система proc](semester2/lab4_proc)
5. [Буферизованный и небуферизованный ввод/вывод](semester2/lab5_open)
6. [Сокеты](semester2/lab6_sockets)
7. [Системный вызов open](semester2/lab7_open/report.pdf)
8. [Виртуальная файловая система](semester2/lab8_vfs)
9. [Обработчики прерываний](semester2/lab9_interrupts)
